...
import { CreateFloorInput } from '../dto/create-parking-floor.input';

import { CreateSlotInput } from '../dto/create-parking-slot.input';
...
export class ParkingAdminService {
  constructor(
    private readonly lotRepository: ParkingLotRepository,
   ...
   * Create parking lot.
   */
  async createParkingLot(input: CreateParkingLotInput): Promise<ParkingLot> {
   ...
   */
  async createFloor(input: CreateFloorInput): Promise<ParkingFloor> {
    const lot = await this.lotRepository.findById(input.parkingLotId);
   ...
   * Create single slot.
   */
  async createSlot(input: CreateSlotInput): Promise<ParkingSlot> {
  ...
  async generateSlots(
    ...
    return this.slotRepository.saveMany(slots);
  }
}
